...
| ID | 状态 | 任务 | 参考命令/方法 | 完成标准（DoD） |
| :--- | :--- | :--- | :--- | :--- |
| **T01** | ✅ 已完成（2026-02-16） | 安装与校验开发环境（.NET SDK、CLI） | `dotnet --info` / `dotnet --version` | 本机可稳定执行 .NET CLI，版本与目标框架一致 |
| **T02** | ✅ 已完成（2026-02-16） | 初始化解决方案与项目骨架（Blazor + API + Gateway + Tests） | `dotnet new sln`、`dotnet new blazor`、`dotnet sln add`（明确采用 `.sln` 或 `.slnx`） | 解决方案可一键还原与构建，目录结构固定，团队统一解文件格式 |
| **T03** | ✅ 已完成（2026-02-16） | 锁定 SDK 与模板策略（Blazor） | `dotnet new globaljson --sdk-version <version> --roll-forward latestFeature`、`dotnet new blazor -h`（确认 `--interactivity`） | SDK 版本可复现，首版渲染模式在文档与代码一致（建议 Server/Auto 二选一） |
| **T04** | 🟡 进行中 | 建立代码规范与基础质量门禁 | `.editorconfig`、`dotnet format --verify-no-changes` | 本地与 CI 均可执行格式检查，输出稳定 |
| **T05** | ✅ 已完成（2026-02-16） | 搭建数据库与 Timescale 扩展 | SQL 初始化脚本 + 环境变量配置 | 可创建库、可连通、可写入/查询基础表 |
| **T06** | ⬜ 未开始 | 定义核心数据模型（设备/点位/遥测/告警） | EF Core 实体与配置 | 模型评审通过，字段与业务词汇一致 |
| **T07** | ⬜ 未开始 | 建立迁移流程（开发/测试/生产） | `dotnet ef migrations add`、`dotnet ef database update` | 本地迁移可重复执行；迁移文件纳入版本管理 |
| **T08** | ⬜ 未开始 | 建立生产迁移发布方式（脚本优先） | `dotnet ef migrations script --idempotent` | 能生成可审阅 SQL 脚本并通过预发验证 |
| **T09** | ⬜ 未开始 | 完成 Modbus TCP 驱动封装 | 连接池、寄存器读写、点位映射 | 对仿真设备稳定读写，异常码可追踪 |
| **T10** | ⬜ 未开始 | 完成采集调度与入库链路 | 定时轮询 + 批量写入 | 秒级采集跑通，数据延迟与丢点率可量化 |
| **T11** | ⬜ 未开始 | 实施采集韧性（重试/超时/断线重连） | Polly 策略（限首版最小集） | 常见网络抖动下可自动恢复且不雪崩 |
| **T12** | ⬜ 未开始 | 采集服务 AOT 发布 | `dotnet publish`（AOT 配置） | 生成可部署包，启动时间与内存基线达标 |
| **T13** | ⬜ 未开始 | 实施轻 CQRS（命令/查询分离） | MediatR + FluentValidation | 关键业务路径已分离，未引入复杂 Pipeline |
| **T14** | ⬜ 未开始 | 交付首版业务 API（设备/点位/实时值） | OpenAPI + 集成测试 | 关键接口可用、错误码一致、鉴权策略明确 |
| **T15** | ⬜ 未开始 | 交付遥测聚合与告警基础能力 | SQL 聚合视图 + 阈值规则 | 日/周/月统计正确，告警可确认与恢复 |
| **T16** | ⬜ 未开始 | 交付 2D Blazor 仪表盘 | 图表组件 + 状态卡 + 告警面板 | 首页可展示实时/历史关键指标，无阻塞卡顿 |
| **T17** | ⬜ 未开始 | 交付控制闭环（UI -> API -> Gateway） | 控制命令链路 + 回执 | 控制成功/失败可见，具备操作审计 |
| **T18** | ⬜ 未开始 | 落地最小可观测性 | 结构化日志 + traceId + 关键链路 tracing | 可基于一次故障在 10 分钟内定位到模块级 |
| **T19** | ⬜ 未开始 | 安全与审计最小集 | 本地账号、RBAC、审计日志 | 权限越权用例被拦截，关键操作可追溯 |
| **T20** | ⬜ 未开始 | 自动化测试与回归基线 | `dotnet test` | 核心单测/集成测试通过，失败可复现 |
| **T21** | ⬜ 未开始 | 稳定性与容量验证 | Soak Test + 压测报告 | 连续运行达标，无明显内存/连接泄漏 |
| **T22** | ⬜ 未开始 | 部署、验收与二期规划 | 发布包、运维手册、缺陷清单 | 首版上线可运维，二期范围（3D/BACnet/IR）冻结 |
